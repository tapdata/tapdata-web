export default {
  ...
  dfs_instance_create_qinggenjushuju: '請根據數據量和任務數量選擇合適的Agent規格進行訂閱，訂閱成功後，不可變更',
  ...
  dfs_instance_selectlist_keyixuanzeyi: '可以選擇已有未使用的訂閱創建Agent，可以創建新Agent選擇更多規格',
  ...
  dfs_instance_selectlist_shouquanmashizai: '授權碼是在阿里雲市場購買生成的授權碼',
  ...
  dfs_instance_expired_time_tip2: '1.过期1天后将禁止调度新任务到过期的 Agent 上去运行',
  dfs_instance_expired_time_tip3: '2.旧任务可以继续运行, 但重启任务时将不可重新调度到过期的 Agent 上去运行',
  ...
  dfs_user_center_ninjiangquxiaoding: '您將取消訂閱“{val1}Agent”業務，取消後您將不再享受該服務，確定是否繼續？ ',
  dfs_user_center_tuidingfuwu: '退訂服務',
  dfs_user_center_ninjiangtuidingr: '您將退訂“{val1}”業務，退訂後您將不再享受該服務，確定是否繼續？ ',
  ...
  dfs_agent_download_subscriptionmodeldialog_geyugumeimiao: ' 個, 預估每秒同步的性能在 ',
  dfs_agent_download_subscriptionmodeldialog_keyongneicunjian: ' 可用內存, 建議的任務數不超過',
  dfs_agent_download_subscriptionmodeldialog_guigeshuomingci: '規格說明: 此規格需要至少 ',
  dfs_agent_download_subscriptionmodeldialog_qingxuanzeninxu: '請選擇您需要的產品規格',
  dfs_agent_download_subscriptionmodeldialog_zhijieshiyonga: '直接使用阿里雲市場授權碼',
  dfs_agent_download_subscriptionmodeldialog_gengjiakekaoyou:
    '- 更加可靠: 由 Tapdata 提供資源使用的維護與監控, 運行更可靠',
  dfs_agent_download_subscriptionmodeldialog_gengjiafangbianmian: '- 更加方便: 免部署, 一鍵交付使用',
  dfs_agent_download_subscriptionmodeldialog_shiyongquantuoguan: '使用全託管模式的好處：',
  dfs_agent_download_subscriptionmodeldialog_zaizhezhongmoshi: '在這種模式下, Tapdata 提供所有的計算及存儲資源.',
  ...
  dfs_agent_download_subscriptionmodeldialog_shiyongbantuoguan: '使用半託管模式的好處：',
  dfs_agent_download_subscriptionmodeldialog_zaizhezhongmoshi2: '在這種模式下, 你需要自己提供自己計算資源與存儲資源',
  ...
  dfs_guide_deploy_wanchengdoc: '完成 docker 服務安裝後, 請複制下方命令執行, 執行後, 等待平台檢測引擎啟動',
  dfs_guide_deploy_qingfuzhixiafang2: '請複制下方命令, 在目錄中執行, 執行後, 等待平台檢測引擎啟動',
  dfs_guide_index_bushujisuanyin: '部署計算引擎',
  dfs_guide_index_qingxuanzeninxiang: '請選擇您想通過本產品完成您的什麼需求呢？ ',
  ...
  dfs_components_taskalarmtour_deployment_qingwen: '請問您的源和目標數據庫, 可以從互聯網上直接訪問嗎?',
  dfs_components_taskalarmtour_deployment_bukeyi: '不可以 (需要您部署計算引擎)',
  dfs_components_taskalarmtour_deployment_wodeshujuku: '您的數據庫在局域網內, 或者出於安全考慮, 無法暴露在公共網絡中',
  dfs_components_taskalarmtour_deployment_keyi: '可以 (平台維護計算引擎)',
  dfs_components_taskalarmtour_deployment_yunshujuku: '您的數據庫均為雲數據庫, 且可以暴露到公共網絡',
  ...
  dfs_user_contactus_gongzuorijiejia: '工作日 09:00 - 20:00 節假日 09:00 - 18:00',
  dfs_user_contactus_dianhuazixun: '電話諮詢',
  dfs_user_contactus_weinintigongchan: '為您提供產品配置指導、問題排查等技術支持',
