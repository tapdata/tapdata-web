export default {
  ...
  verify_operation_verify_again_info:
    '可以選擇校驗結果不一致的表進行重新校驗，不選擇時默認對當前所有不一致的表進行校驗。',
  ...
  verify_type_field: '表全字段值校驗',
  verify_type_joint_field: '關聯字段值校驗',
  verify_tips_type_row_count: '快速count僅對源表和目標表的行數進行count校驗，速度極快，但是不會展示差异的具體字段內容',
  verify_tips_type_field: '全表字段值校驗會對源表和目標表的全部字段進行逐行校驗，能查出所有字段的差异，但是速度慢',
  verify_tips_type_joint_field: '關聯鍵校驗只對源表和目標表的關聯字段的值進行比對校驗，速度快於全表字段值校驗模式',
  ...
  verify_message_error_joint_table_field_not_set: '校驗條件中源表或目標表的索引字段未選擇',
  verify_message_error_joint_table_field_not_match: '校驗條件中源表與目標表的索引字段個數不相等',
  ...
  dataVerification_success: '恭喜~~~~校驗結果源表與目標表內容完全一致，沒有錯誤記錄',
  ...
  verify_Form_zhaoBuDaoJieDian: '找不到節點對應的表資訊',
