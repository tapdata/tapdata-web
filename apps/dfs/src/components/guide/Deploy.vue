<template>
  <div class="account">
    ...
    <div class="fw-sub font-color-dark mt-4">
      {{ $t('dfs_select_server_type') }}
    </div>
    <ElRadioGroup v-model:value="downLoadType" class="flex gap-4 mt-4 mb-4">
      <ElRadio
...
export default {
  ...
  methods: {
    ...
    onCopy() {
      ...
      $emit(this, 'behavior', `copy_${this.downLoadType}`)
    },
    handleCopy() {
      const MAP = {
    ...
    handleDownLoad() {
      ...
      $emit(this, 'behavior', `download_agent`)
    },
    //windows 下载
