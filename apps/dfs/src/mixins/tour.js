...
export default {
  ...
  methods: {
    ...
    async checkGuide() {
      ...
      items = items.filter(
        ({ id }) =>
          !['64a785ada8321f670d2338d1', '64a01d43a1dd0e614a1b7d86'].includes(id)
      )
      ...
      if (subscribeId) {
        let isUnPay = subItems.find(
          (i) => i.status === 'incomplete' && guide.subscribeId === i.id
        )
    ...
    async initAgentTour() {
      ...
      const element = `#agent-${agent.id} [name="${
        agent.agentType === 'Cloud' ? 'restart' : 'start'
      }"]`
        ...
        (to) => {
          if (
            ...
            if (!this.$store.state.replicationConnectionDialog) {
              this.$nextTick(() => {
                this.replicationDriverObj.drive(
                  this.replicationTour.activeIndex || 0
                )
              })
            }
          } else {
