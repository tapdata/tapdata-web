          ...
          <div class="panel" v-if="$route.params.id && agentInfo">
            ...
            <ul class="info-list">
              ...
              <li class="info-item">
                ...
                <span class="value">{{ agentInfo.createAtFmt }}</span>
              </li>
  ...
  data() {
    return {
      ...
      form: {
        ...
        name: 'DRS_' + dayjs().format('YYYYMMDDHHmmss') + Math.floor(Math.random() * 16 * 1000),
        serviceId: '',
        groupId: '',
        measureId: '',
        periodType: 'month',
  ...
  methods: {
    getInstance(id, regionMap, zoneMap) {
      this.$axios.get('api/tcm/agent/' + id).then(data => {
        ...
        data.createAtFmt = dayjs(data.createAt).format('YYYY-MM-DD HH:mm:ss')
        this.agentInfo = data
        ...
        this.agentInfo.endTimeStr = data.endTime ? dayjs(data.endTime).format('YYYY-MM-DD') : ''
      })
    },
    getRegion(cb) {
      this.$axios
