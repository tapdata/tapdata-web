  ...
  <div v-if="$route.name === 'Workbench'" class="workbench-container">
    ...
    <el-dialog :visible.sync="showUpgrade" title="版本升级通知">
      <div class="dialog-upgrade">
        <div>
          <p class="mb-2 dialog-upgrade__header">尊敬的用户,您好：</p>
        ...
        <p class="mb-2 mt-2 dialog-upgrade__header">下面是新版本增加的特性：</p>
        <p class="mt-4 mb-2 dialog-upgrade__header">基于PDK的数据源注册机制</p>
        <div>
          ● 基于PDK来实现数据源的开发<br />
          ● 用户可按照PDK的标准灵活定制自己的数据源<br />
          ● 新开发的数据源可以通过注册快速接入平台，无需重启即可使用<br />
        </div>
        <p class="mt-4 mb-2 dialog-upgrade__header">全量和增量阶段的数据一致性校验能力</p>
        <div>
          ● 具备对数据复制任务全量数据一致性的校验能力<br />
          ● 具备对数据复制任务增量数据一致性的实时校验能力<br />
          ● 具备对数据校验的差异结果发起再次校验的能力<br />
        </div>
        <p class="mt-4 mb-2 dialog-upgrade__header">任务可观测能力</p>
        <div>
        ...
        <p class="mt-4 mb-2 dialog-upgrade__header">数据同步能力增强</p>
        <div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="showUpgrade = false">取 消</el-button>
      </span>
    </el-dialog>
  </div>
