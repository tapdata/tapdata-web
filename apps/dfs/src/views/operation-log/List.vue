...
export default {
  ...
  methods: {
    ...
    getData({ page }) {
      ...
      let { operationType, parameter1, start, end, username } =
        this.searchParams
      let where = {
      ...
      return this.$axios
        .get(
          'tm/api/UserLogs?filter=' + encodeURIComponent(JSON.stringify(filter))
        )
        .then(({ total, items }) => {
          return {
            total: total,
    ...
    descFnc(row) {
      ...
      let findOne = this.operationTypeOptions.find(
