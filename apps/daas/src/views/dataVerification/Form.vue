  ...
  <section class="verify-form-wrap section-wrap flex-fill" v-loading="loading">
    <div class="section-wrap-box">
      ...
      >
        ...
        <div class="joint-table-footer">
          <ElButton @click="addTable()">{{ $t('verify_button_add_table') }}</ElButton>
          <ElButton type="primary" @click="autoAddTable()">{{ $t('verify_button_auto_add_table') }}</ElButton>
        </div>
      </div>
      <div class="mt-8">
        <ElButton @click="goBack()">{{ $t('button_back') }}</ElButton>
        <ElButton type="primary" :disabled="!flowStages || !flowStages.length" @click="nextStep()">{{
          $t('button_save')
        }}</ElButton>
      </div>
    </div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <ElButton @click="handleAddScriptClose">{{ $t('button_cancel') }}</ElButton>
        <ElButton type="primary" @click="submitScript">{{ $t('button_confirm') }}</ElButton>
      </span>
    </ElDialog>
  </section>
</template>
