  ...
  if (rsp) {
    ...
    switch (rsp.status) {
      ...
      case 401:
        signOut()
        setTimeout(() => {
          Message.error({ message: i18n.global.t('message_401').toString() })
        }, 500)
        break
      // 请求的资源不存在
      case 404:
        Message.error({ message: i18n.global.t('message_404').toString() })
        break
      case 504:
        Message.error({ message: i18n.global.t('message_5xx').toString() })
        break
      case 500:
        Message.error({ message: i18n.global.t('message_5xx').toString() })
        break
    ...
    Message.error({
      message: i18n.global.t('message_network_unconnected').toString()
    })
  } else if (error.message && error.message.includes('timeout')) {
    Message.error({
      message: i18n.global.t('message_request_timeout').toString()
    })
  }
  return Promise.reject(error)
}
...
axios.interceptors.response.use((response: AxiosResponse) => {
  return new Promise((resolve, reject) => {
    ...
    if (code === 'ok') {
      ...
      Message.error(response.data.message || i18n.global.t('message_request_error').toString())
      reject(response)
    } else {
      switch (code) {
        case 'SystemError':
          if (data.message === 'System error: null') {
            Message.error({
              message: i18n.global.t('message_request_error').toString()
            })
          } else {
            Message.error({
