...
cp.exec(`git config --remove-section alias`, function () {
  fs.readFile(join(__dirname, '../.git/config'), 'utf8', function (err, data) {
    ...
    lines.push(
      ...
      "\tsinit = submodule foreach 'git checkout -B $(git -C ../../../../ rev-parse --abbrev-ref HEAD)-webcore && git config -f ../../../../.gitmodules submodule.frontend-el/src/_packages/tapdata-web-core.branch $(git rev-parse --abbrev-ref HEAD)'",
      "\tsbranch = submodule foreach 'git config -f ../../../../.gitmodules submodule.frontend-el/src/_packages/tapdata-web-core.branch'",
      '\tsu = submodule update'
    )
