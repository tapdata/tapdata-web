...
export function useFormScope() {
  ...
  const findParentNodeByType = (node: Record<string, any>, type: string) => {
    const parentIds = node?.$inputs || []
    ...
    for (const pid of parentIds) {
      ...
      return findParentNodeByType(parent, type)
    }
  }

  const loadNodeFieldsById = async (nodeId) => {
    if (!nodeId) return []
    try {
  ...
  const loadDateFieldOptions = async (nodeId) => {
    const fields = await loadNodeFieldsById(nodeId)
    const set = new Set()
    fields.forEach((f) => {
  ...
  const loadNodeFieldOptions = async (nodeId) => {
    const fields = await loadNodeFieldsById(nodeId)
    return fields
  ...
  const scope = {
    ...
    $isMonitor: ['MigrationMonitor', 'TaskMonitor'].includes(route.name),

    formTab: FormTab.createFormTab(),

    $hasPdkConfig: (pdkHash) => {
      return !!dataflowStore.pdkPropertiesMap[pdkHash]
    },
    ...
    usePdkFieldDateType: () => {
      return async (field) => {
        ...
        const [parent] = findParentNodes($values.id)
    ...
    setDefaultPrimaryKey(field) {
      if (!field.value?.length && field.dataSource && field.dataSource.length) {
        ...
        field.setValue(
          (isPrimaryKeyList.length ? isPrimaryKeyList : indicesUniqueList).map(
            (item) => item.value,
          ),
        )
        field.validate()
    ...
    async addEditorFieldCompletion(editor, nodeId, $inputs, isMigrate) {
      ...
      const idx =
        editor.completers?.findIndex((item) => item.id === 'recordFields') || -1
    ...
    validateUpdateConditionFields: async (value, rule, ctx) => {
      ...
      const nodeData = findNodeById($values.id)
      ...
      if ($values.attrs.hasCreated === false) {
        if (!options || !options.length) {
          options = await loadNodeFieldOptions($values.id)
        }
    ...
    changeEnableUpdateJoinKeyValue(val, field) {
      ...
      // 如果是一级开关
      if (
        /mergeProperties\.\d+\.enableUpdateJoinKeyValue/.test(field.path.entire)
      ) {
