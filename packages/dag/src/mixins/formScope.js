      ...
      scope: {
        ...
        setDefaultPrimaryKey(field) {
          if (
            ...
            let isPrimaryKeyList = field.dataSource.filter(
              (item) => item.isPrimaryKey
            )
            let indicesUniqueList = field.dataSource.filter(
        ...
        validateUpdateConditionFields: async (value, rule, ctx) => {
          ...
          console.debug(
            'validateUpdateConditionFields.ctx',
            ctx,
            $values.attrs.hasCreate
          ) // eslint-disable-line
          if (!$values.$inputs[0]) {
            return
          }
          ...
          if ($values.attrs.hasCreated === false) {
            if (!options || !options.length) {
              options = await this.scope.loadNodeFieldOptions(
                $values.$inputs[0]
              )
            }
            ...
            if (options && options.length) {
              ...
              let indicesUniqueList = options.filter(
                (item) => item.indicesUnique
              )
              let defaultList = (
                isPrimaryKeyList.length ? isPrimaryKeyList : indicesUniqueList
              ).map((item) => item.value)
