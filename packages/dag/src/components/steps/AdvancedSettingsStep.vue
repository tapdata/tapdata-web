  ...
  setup(props, { emit }) {
    ...
    const scope = {
      ...
      getConnectionNameByAgent: (field) => {
        ...
        const map = {
          [taskRef.value.dag.nodes[0].connectionId]:
            taskRef.value.dag.nodes[0].attrs.connectionName,
          [taskRef.value.dag.nodes[3].connectionId]:
            taskRef.value.dag.nodes[3].attrs.connectionName,
        }
    ...
    const initTaskAgent = () => {
      ...
      if (size >= 1) {
        ...
        currentId =
          currentId && accessNodeProcessIdArr.value.includes(currentId)
            ? currentId
            : accessNodeProcessIdArr.value[0]
        taskRef.value.accessNodeType =
    ...
    const accessNodeProcessIdMap = computed(() => {
      return [taskRef.value.dag.nodes[0], taskRef.value.dag.nodes[3]].reduce(
        (map, node) => {
    ...
    const accessNodeProcessList = computed(() => {
      const agents = scope.$agents.filter(
        (item) => item.accessNodeType === taskRef.value.accessNodeType,
      )
                            ...
                            syncIndexEnable: {
                              ...
                              'x-reactions': {
                                fulfill: {
                                  state: {
                                    visible:
                                      '{{hasFeature("syncIndex") && $settings.type !== "cdc"}}',
...
<template>
  <div
    class="position-relative flex flex-column gap-4 h-100 min-h-0 rounded-lg overflow-y-auto"
  >
