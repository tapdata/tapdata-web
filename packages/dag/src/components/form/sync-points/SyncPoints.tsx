...
export const SyncPoints = defineComponent({
  ...
  setup(props) {
    ...
    reactiveWatch(
      () => props.value,
      (v: any) => (items.value = v?.filter((item: any) => !!item.nodeId) || []),
    )

    const checkStreamOffset = (nodeId: string) => {
      return dataflowStore.hasCapability(
        dataflowStore.findNodeById(nodeId),
    ...
    const hasStreamOffsetItem = () =>
      items.value.filter(
        (item: any) => !item.hiddenPointType && checkStreamOffset(item.nodeId),
      ).length > 1
