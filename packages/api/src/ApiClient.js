...
export default class ApiClient {
  ...
  async getAPIServerToken() {
    ...
    const result = await this.axios.create().post('/oauth/token', data)
  ...
  async loadOpenAPI() {
    ...
    try {
      ...
      let response = await this.axios.create().get(openAPIUrl)
  ...
  /* async get (params) {
    const request = this.axios.create({headers: {token: await this.getAPIServerToken()}})
    if (params.url) {
      let url = params.url
      delete params.url
