...
import {
  composeExport,
  resolveComponent,
} from '@formily/element/lib/__builtins__/shared'
...
const takeUnitVal = (val) =>
  val === 'auto' ? undefined : isNaN(val) ? val : `${val}px`
    ...
    return () => {
      ...
      if (labelWidth || wrapperWidth) {
        if (labelWidth) {
          ...
          labelStyle.maxWidth = width
        }
        if (wrapperWidth) {
          const width = takeUnitVal(wrapperWidth)
