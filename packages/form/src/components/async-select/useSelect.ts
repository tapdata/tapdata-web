...
import { useInput } from 'element-plus/es/components/select-v2/src/useInput'
...
import type { ISelectProps, SelectOptionProxy } from 'element-plus/es/components/select/src/token'
...
export const useSelect = (props: ISelectProps, emit) => {
  ...
  watch(
    ...
    (val) => {
      if (val) {
        ...
        if (props.lazy && !states.options.size) {
          loadData()
        }
      } else {
        states.inputValue = ''
  ...
  const setSelected = async () => {
    if (!props.multiple) {
      const option = await getOption(props.modelValue, false, props.onSetSelected && states.hoveringIndex > -1)
  ...
  const emitChange = (val) => {
    if (!isEqual(props.modelValue, val)) {
      ...
      if (props.multiple) {
        ...
        emit('change-label', changeLabels)
      } else {
        emit('change-label', states.options.get(val).currentLabel)
      }
    }
  }
