<template>
  <div>
    <el-button
      ...
      :disabled="isDisable"
      @click="getMetaData()"
      >{{ $t('packages_fieldMapping_dag_link_button_field_mapping') }}</el-button
    >
    <el-dialog
      v-if="dialogFieldProcessVisible"
      width="85%"
      :title="$t('packages_fieldMapping_dag_link_button_mapping_configuration')"
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" v-if="this.transform.mode !== 'readOnly'" @click="saveReturnData">{{
          $t('packages_fieldMapping_dataVerify_confirm')
        }}</el-button>
      </div>
    </el-dialog>
  </div>
    ...
    getMetaData() {
      ...
      if (this.transform?.syncType === 'migrate') {
        ...
        if (
          ...
          this.$message.error(this.$t('packages_fieldMapping_dag_link_field_mapping_error_no_table'))
          return
        } else if (this.transform.selectSourceArr?.length === 0 && !this.transform.transferFlag) {
          this.$message.error(this.$t('packages_fieldMapping_dag_link_field_mapping_error_no_table')) //其他数据源
          return
        }
      }
      this.loading = true
