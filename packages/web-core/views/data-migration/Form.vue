...
import TCM from 'web-core/api/TCM'
import DataFlows from 'web-core/api/DataFlows'
import Connections from 'web-core/api/Connections'
import MetadataInstances from 'web-core/api/MetadataInstances'

const DataFlowsModel = new DataFlows()
const TcmModel = new TCM()
const ConnectionsModel = new Connections()
const MetadataInstancesModel = new MetadataInstances()
...
export default {
        ...
        rules: [
          {
            ...
            validator: (rule, value, callback) => {
              if (!value || !value.trim()) {
                ...
                DataFlowsModel.get(JSON.stringify({ filter })).then(({ items }) => {
                  if (items?.length !== 0) {
  ...
  methods: {
    ...
    //云版支持数据源
    allowDatabaseType() {
      ConnectionsModel.getAllowDatabaseType().then(data => {
    ...
    getConnection(where, type, reset = false) {
      ...
      return ConnectionsModel.get(JSON.stringify({ filter })).then(({ items }) => {
    ...
    showFieldMapping() {
      ...
      DataFlowsModel.tranModelVersionControl(param).then(data => {
    ...
    save() {
      ...
      if (this.id) {
        postData['id'] = this.id
        promise = DataFlowsModel.updateById(this.id, postData)
      } else {
        promise = DataFlowsModel.post(postData)
      }
      this.loading = true
      promise
        .then(() => {
    ...
    async findOneConnection(id) {
      ...
      let result = await ConnectionsModel.get(JSON.stringify({ filter }))
