...
import TypeMapping from 'web-core/api/TypeMapping'

const DataFlowsModel = new DataFlows()
const MetadataInstancesModel = new MetadataInstances()
const TypeMappingModel = new TypeMapping()
  ...
  methods: {
    ...
    getMetaData(taskData) {
      ...
      if (taskData?.metadataMappings?.length > 0) {
        ...
        DataFlowsModel.metadata(taskData)
          .then(data => {
    ...
    async updateFieldProcess(rollback, rollbackTable, id) {
      ...
      return await DataFlowsModel.metadata(this.dataFlow)
    },
    ...
    async intiFieldMappingTableData(row) {
      let source = await DataFlowsModel.originalData(row.sourceQualifiedName)
      source = source && source.length > 0 ? source[0].fields : []
      let target = await DataFlowsModel.originalData(row.sinkQulifiedName)
      // 初始化所有字段都映射 只取顶级字段
    ...
    async getTypeMapping(row) {
      return await TypeMappingModel.dataType(row.sinkDbType)
    },
    //保存字段映射操作
