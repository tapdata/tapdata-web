...
function getScrollDetails(parent, child, beforeRef, afterRef, horizontal, rtl, stickyStart, stickyEnd) {
  ...
  if (horizontal === true) {
    ...
    if (rtl === true) {
      details.scrollStart =
        (buggyRTL === true ? details.scrollMaxSize - details.scrollViewSize : 0) - details.scrollStart
    }
  } else {
    if (parent === window) {
...
export default {
  ...
  computed: {
    ...
    colspanAttr() {
      return this.tableColspan !== void 0 ? { colspan: this.tableColspan } : { colspan: 100 }
    }
  },
  ...
  methods: {
    ...
    scrollTo(toIndex, edge) {
      ...
      this.__scrollViewSize !== scrollDetails.scrollViewSize &&
        this.__setVirtualScrollSize(scrollDetails.scrollViewSize)
    ...
    __onVirtualScrollEvt() {
      ...
      const scrollDetails = getScrollDetails(
        ...
        listEndOffset =
          ...
          this.virtualScrollPaddingAfter
    ...
    __setVirtualScrollSliceRange(scrollEl, scrollDetails, toIndex, offset, align) {
          ...
          Math.ceil(
            toIndex -
              this.virtualScrollSliceSizeComputed /
                (alignEnd === void 0 || alignEnd === 'center' ? 2 : alignEnd === 'start' ? 3 : 1.5)
          )
        ),
      ...
      const hadFocus =
        rangeChanged === true && typeof scrollEl.contains === 'function' && scrollEl.contains(document.activeElement)
      ...
      if (rangeChanged === true) {
        ...
        this.virtualScrollPaddingAfter = sumSize(
      ...
      requestAnimationFrame(() => {
        ...
        if (alignEnd !== void 0) {
          ...
          scrollPosition =
            alignForce !== true && scrollStart < posStart && posEnd < scrollStart + scrollDetails.scrollViewSize
              ? scrollStart
              : alignEnd === 'end'
              ...
              : posStart -
                (alignEnd === 'start'
                  ? 0
                  : Math.round((scrollDetails.scrollViewSize - this.virtualScrollSizes[toIndex]) / 2))
        }

        this.prevScrollStart = scrollPosition

        setScroll(scrollEl, scrollPosition, this.virtualScrollHorizontal, rtl)
    ...
    __updateVirtualScrollSizes(from) {
      ...
      if (contentEl !== void 0) {
        const children = slice
            .call(contentEl.children)
            .filter(el => el.classList.contains('q-virtual-scroll--skip') === false),
          childrenLength = children.length,
          sizeFn =
            this.virtualScrollHorizontal === true ? el => el.getBoundingClientRect().width : el => el.offsetHeight
    ...
    __resetVirtualScroll(toIndex, fullReset) {
      ...
      if (toIndex >= 0) {
        ...
        this.virtualScrollPaddingBefore = sumSize(
          this.virtualScrollSizesAgg,
          this.virtualScrollSizes,
          0,
      ...
      return [
        tag === 'tbody'
          ? h(
              tag,
              {
                staticClass: 'virtual-scroll__padding',
                key: 'before',
                ref: 'before'
              },
              [
        ...
        h(
          tag,
          {
            staticClass: 'virtual-scroll__content',
            key: 'content',
            ref: 'content'
          },
          content
  ...
  beforeMount() {
    ...
    this.__onVirtualScrollEvt = debounce(this.__onVirtualScrollEvt, os.isIPhone === true ? 120 : 35)
    this.__setVirtualScrollSize()
  }
}
