<template>
  ...
  >
    ...
    >
      ...
      <!-- 基础信息 -->
      <ul
        class="flex flex-wrap bg-subtle pt-3 px-1 rounded-4 data-server-form-group"
      >
        <li class="data-server-form-base__item px-2">
          <ElFormItem
            :label="$t('packages_business_caozuoleixing')"
            label-width="86px"
          >
        ...
        <li class="data-server-form-base__item px-2">
          <ElFormItem
            :label="$t('packages_business_fabujiedian')"
            label-width="86px"
          >
        ...
        <li class="data-server-form-base__item px-2">
          <ElFormItem
            :label="$t('packages_business_jiekouleixing')"
            label-width="86px"
          >
            <ElSelect
              v-if="isEdit"
      ...
      <section v-if="tab === 'form'">
        <div class="mt-4 fs-7 data-server-panel__title">
          {{ $t('packages_business_aPI_path_Settings') }}
        </div>
        <div class="flex-1 mt-4" size="small">
          ...
          >
            <el-radio label="default">{{
              $t('packages_business_default_path')
            }}</el-radio>
            <el-radio label="customize">{{
              $t('packages_business_custom_path')
            }}</el-radio>
          </el-radio-group>
        </div>
        <ElFormItem
      ...
      <ElTable class="flex-1" :data="isEdit ? form.params : data.params">
        <ElTableColumn
          :label="$t('packages_business_canshumingcheng')"
          prop="name"
          min-width="120"
        >
        ...
        <ElTableColumn
          v-if="debugParams"
          :label="$t('packages_business_canshuzhi')"
          min-width="100"
        >
      ...
      <template v-if="tab === 'form'">
        <div class="data-server-panel__title">
          {{ $t('packages_business_shuchujieguo') }}
        </div>
        <ElTable
          ref="fieldTable"
      ...
      <template v-if="tab === 'debug'">
        <div class="data-server-panel__title">
          {{ $t('packages_business_fanhuijieguo') }}
        </div>
        <VCodeEditor
          height="280"
          lang="json"
        ...
        if (valid) {
          ...
          if (params.some((it) => !it.name.trim())) {
            return this.$message.error(
              i18n.t('packages_business_qingshurucanshu')
            )
          }
          ...
          $emit(this, 'save', data, this.params.to)
          this.isEdit = false
        }
      })
    },
