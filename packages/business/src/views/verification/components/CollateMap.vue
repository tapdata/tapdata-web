<template>
  <div class="py-3 flex flex-column gap-3">
    <div v-for="(item, i) in list" :key="item.key" class="flex gap-3">
      <FieldSelect v-model="item.key" :options="sortFields" placeholder="请选择字段" @change="handleChange" />
      <ElInput v-model="item.value" placeholder="请输入字符集" @change="handleChange" />
