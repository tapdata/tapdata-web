<template>
  <div class="overview-bar">
    <div class="mb-4 fs-7 font-color-dark">
      {{ this.$t('packages_business_task_monitor_full_sync') }}
    </div>
    <div class="overview-bar__content p-4">
      <div class="flex justify-content-between mb-2 font-color-dark">
        <div>
          <span
            >{{ $t('packages_business_task_info_table_number') }} :
            {{ totalDataText }}</span
          >
          <span class="ml-3"
            >{{ $t('packages_business_task_info_completed') }} :
            {{ info.finishNumber || 0 }}</span
          >
        </div>
        ...
        <div v-else>
          {{ $t('packages_business_task_info_fully_completed') }}
        </div>
      </div>
      <ElProgress
        color="#2C65FF"
...
export default {
  ...
  computed: {
    totalDataText() {
      ...
      return num < 0
        ? this.$t('packages_business_task_info_overView_status')
        : num || 0
    },
    finishDurationText() {
