...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const pageCount = computed(() => {
      const current = page.value.current || 1
      return Math.max(1, current + (hasNextPage.value ? 1 : 0))
    })
    ...
    const fetch = (
      ...
      const prevCurrent = page.value.current
      const prevSkip = page.value.skip
      const prevList = list.value
      const targetCurrent = Math.max(1, pageNum || page.value.current)
      const direction =
        targetCurrent > prevCurrent
          ? 'next'
          : targetCurrent < prevCurrent
            ? 'prev'
            : 'same'

      if (direction !== 'same' || targetCurrent === 1) {
      ...
      page.value.current = targetCurrent
      page.value.skip = (targetCurrent - 1) * page.value.size
      ...
      const run = () => {
        ...
        props
          ...
          .then(({ data }: { data: any[] }) => {
            ...
            if (!nextList.length && direction === 'next') {
              hasNextPage.value = false
              page.value.current = prevCurrent
              page.value.skip = prevSkip
              list.value = prevList
              return
            }

            if (!nextList.length && targetCurrent === 1) {
              hasNextPage.value = false
      ...
      if (debounce > 0) {
        debounceTimer.value = setTimeout(run, debounce)
        return
      }
