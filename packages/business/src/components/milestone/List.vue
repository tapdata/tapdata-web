    ...
    <div v-if="activeNodeId" class="flex-fill overflow-auto">
      ...
      >
        <template v-slot:statusLabel="scope">
          ...
          >
            <span
              class="color-danger underline cursor-pointer"
              @click="handleError(scope.row)"
              >{{ $t('packages_business_milestone_list_renwuchucuo') }}</span
            >
          </div>
    ...
    <div v-else class="flex-fill overflow-auto py-4">
      ...
      >
        <div class="position-relative">
          ...
          ></div>
          <VIcon :class="[item.color, 'mt-1 position-relative']" size="16">{{
            item.icon
          }}</VIcon>
        </div>
...
export default {
      ...
      result.forEach((el) => {
        if (el.key === 'FULL_SYNC') {
          ...
          if (snapshotDoneAt) {
            ...
            const { progress, time } = this.getDueTimeAndProgress(
              this.totalData
            )
        ...
        .filter(
          (t) =>
            ...
            (dataflowType === 'initial_sync' &&
              !['OPEN_STREAM_READ', 'STREAM_READ'].includes(t.key))
        )
        .map((el) => {
  ...
  methods: {
    ...
    handleError(row = {}) {
      this.$confirm(
        row.errorMessage,
        i18n.t('packages_business_milestone_list_cuowuxinxi'),
        {
          type: 'warning',
