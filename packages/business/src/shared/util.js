...
export const formatMs = (msTime = 0, type = 'time') => {
  ...
  let arr = []
  arr.push({
    label: i18n.global.t('packages_business_task_info_d'),
    value: Math.floor(time / 60 / 60 / 24)
  }) // day
  arr.push({
    label: i18n.global.t('packages_business_task_info_h'),
    value: Math.floor(time / 60 / 60) % 24
  }) // hour
  arr.push({
    label: i18n.global.t('packages_business_task_info_m'),
    value: Math.floor(time / 60) % 60
  }) // minute
  arr.push({
    label: i18n.global.t('packages_business_task_info_s'),
  ...
  if (!result) {
    result = msTime + i18n.global.t('packages_business_task_info_ms')
  }
  return result
}
