...
export function calcQuadrantOfPointToRect(point: IPoint, rect: IRect) {
  ...
  } else if (point.y <= rect.y + rect.height / 2) {
    if (isInner) {
...
export function calcCombineSnapLineSegment(
  ...
  if (target.start.x === target.end.x) {
    return new LineSegment(
      new Point(target.start.x, Math.min(target.start.y, source.start.y)),
      new Point(target.start.x, Math.max(target.end.y, source.end.y)),
    )
  }

  return new LineSegment(
    new Point(Math.min(target.start.x, source.start.x), target.start.y),
    new Point(Math.max(target.end.x, source.end.x), target.end.y),
  )
}
