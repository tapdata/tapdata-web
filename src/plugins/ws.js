...
class WSClient {
  ...
  connect() {
    ...
    try {
      ...
      // let token = 'mAkyJPauFQr0XzK06MJMRaROK1fET57xxpzOKTqHazjEufdOh5ZJsdHOMwOcyO6r';
  ...
  handlerClose() {
    console.log('Websocket is closed, ready to reconnect.')
    this.reconnect()
  }
  ...
  handlerMessage(e) {
    ...
    if (typeof msg === 'string' && /^"?\{.*\}"?$/.test(msg)) {
      try {
        ...
        console.log(`Parse message fail: ` + msg, e)
      }
    } else {
  ...
  getAgentId(cb) {
    ...
    if (self.agentId) {
      ...
      this.$axios
        .get('/api/Workers/availableAgent')
        .then(result => {
          console.log('ws.getAgentId:', result)
          if (
            result &&
            result.data &&
            result.data.result &&
  ...
  getUrl() {
    ...
    // host = 'localhost:3030';
    return `${
      location.protocol.indexOf('https') === 0 ? 'wss:' : 'ws:'
    }//${host}/ws/agent`
  }
}
