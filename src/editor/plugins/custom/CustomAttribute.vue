  ...
  <div class="nodeBody">
    <el-form class="e-form" label-position="top" :model="model" ref="form">
      ...
      <el-form-item :label="$t('editor.choose') + 'Custom'" prop="connectionId" :rules="rules" required>
        <el-select filterable v-model="model.connectionId" :placeholder="$t('editor.cell.data_node.custom.chooseCustomName')">
          <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        connectionId: [
          {required: true, trigger: 'blur', message: this.$t('editor.cell.data_node.custom.chooseCustomName')},
        ]
      },
      model: {
        connectionId: "",
