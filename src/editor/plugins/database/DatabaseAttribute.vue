...
export default {
    ...
    computedTables() {
      if (this.search) {
        return this.tables.filter(
          t =>
            t.table_name.toLowerCase().indexOf(this.search.toLowerCase()) >= 0
        );
      } else {
        return this.tables;
      }
  ...
  methods: {
    ...
    removeTable(item, idx) {
      item.checked = false;
      let tableIndex = this.tables.findIndex(
    ...
    undotble(item, idx) {
      item.checked = false;
      let tableIndex = this.removeTables.findIndex(
        table => table.table_name === item.table_name
      );
