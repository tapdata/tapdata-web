  ...
  >
    <div class="action-buttons">
      ...
      <div
        class="headImg"
        @click="reloadSchema"
        :title="$t('dataFlow.button.reloadSchema')"
      >
      ...
      >
        ...
        <span
          class="text"
          v-if="sync_type ==='initial_sync+cdc'"
        >{{ $t("dataFlow.initial_sync") + "+" + $t("dataFlow.cdc")}}</span>
        <span
          class="text"
          v-if="sync_type ==='initial_sync'"
        >{{ $t("dataFlow.initial_sync")}}</span>
        <span
          class="text"
          v-if="sync_type ==='cdc'"
        >{{ $t("dataFlow.cdc")}}</span>
    ...
    >
      ...
      >
        ...
        <el-button
          ...
          type="primary"
          @click="start"
        >{{ $t("dataFlow.submitExecute") }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
      ...
      dataFlowsApi
        ...
        .then(result => {
          if (result && result.data) {
            ...
            if (
              ["scheduled", "running", "stopping", "force stopping"].includes(
                self.status
              )
            ) {
              self.setEditable(false);
