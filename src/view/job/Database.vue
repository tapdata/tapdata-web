    ...
    <div class="nodeBody">
      <el-form class="e-form" label-position="top" label-width="160px" :model="model" ref="form">
        <el-form-item class="e-form" :label="$t('editor.cell.data_node.database.form.label')" prop="connectionId" :rules="rules" required>
          <el-select filterable v-model="model.connectionId" @change="getSelectType" :placeholder="$t('editor.cell.data_node.database.form.placeholder')" size="mini">
            <el-option
...
<script>
	...
	export default {
		...
		async mounted() {
      console.log("数据ku",this.connection_type)
    ...
    watch: {
      ...
      removeTable(item,index) {
        ...
        console.log("单个移除",removeTable, item,index,this.model.excludeTables,this.tables)
      },
      ...
      checkedOne (item,index) {
        ...
        console.log("单选",item,this.checkList)
      },
      ...
      checkedAll (val) {
        ...
        console.log("群选",val,this.checkList)
  ...
  .processingBody {
    ...
    .el-tabs {
      ...
      .list {
        ...
        .el-checkbox { opacity: 0; visibility: hidden;}
      }
      .list {
        .iconfont { color: #4AAF47;}
        .tableName { color: #48b6e2;}
        background-color: #E7F5FA;
        .el-checkbox { opacity: 1; visibility: visible;}
        .el-button--text { opacity: 1; visibility: visible;}
      }
    }
  }
  .databaseInfo {
