	...
	<div class="e-schema-editor" :style="width > 0 ? `width: ${width}px;` : ''" ref="entityDom">
		<el-container>
			...
			<div class="header-row">
				<div class="e-col">{{$t('editor.cell.processor.field.form.fieldName')}}</div>
				<div class="e-col">{{$t('editor.cell.processor.field.form.fieldType')}}</div>
			...
			<el-main>
				<el-tree
					...
					<span class="custom-tree-node" slot-scope="{ node, data }">
						...
						<el-dropdown v-else size="mini" :show-timeout="10" :hide-on-click="false" @command="command => handleCreate(command, node, data)">
						...
						<el-button type="text" v-if="originalSchema.type ==='collection' && data.primary_key_position > 0 " class=" e-field-action iconfont icon-return" disabled></el-button>
						<span v-else class="e-field-action iconfont icon-return" @click="handleReset(node,data)"></span>
		...
		<el-dialog :title=" $t('editor.cell.processor.field.form.scriptDialogTitle') + ' (' + scriptDialog.tableName + '[ ' + scriptDialog.fieldName + ' ])'" :visible.sync="scriptDialog.open" append-to-body>
			<el-form>
				<el-form-item>
