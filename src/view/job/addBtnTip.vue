	...
	<div class="fixBtn">
		<el-popover popper-class="popperFixbtn" placement="top" width="400" trigger="hover">
			<div class="btnList">
				...
				<span>
					{{ $t('dataFlow.amplification') }} <i>{{ isMac ? 'command+plus' : 'ctrl+plus' }}</i>
				</span>
				<span>
					{{ $t('dataFlow.zoomOut') }} <i>{{ isMac ? 'command+minus' : 'ctrl+minus' }}</i>
				</span>
				<span> {{ $t('dataFlow.down') }} <i>key down</i> </span>
				<span> {{ $t('dataFlow.up') }} <i>key up</i> </span>
				<span>
					{{ $t('dataFlow.selectMultipleNode') }} <i>Shift+{{ $t('dataFlow.mouseDrag') }}</i>
				</span>
			</div>
