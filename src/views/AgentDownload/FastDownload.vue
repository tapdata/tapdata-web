  ...
  <section :class="['fast-download', 'down-page', $PLATFORM]">
    ...
    <main class="page-main block">
      ...
      <p class="title-text pt-10">
        Tapdata DFS 云版需要先在本地安装 Agent
        以确保连接数据库和数据传输服务的正常运行,
        ...
        <ul class="pt-5 ul-style">
          ...
          <li>
            <ElButton class="mt-5" type="primary" @click="handleDownLoad"
              >点击下载 Tapdata Agent</ElButton
            >
          </li>
          <li class="mt-5">
            3. 在 Tapdata Agent
            下载完毕后，请将其放入任意目录并执行下方命令即可实现 Tapdata Agent
            的自动部署及启动
          </li>
          <li>
            <h4 class="s-title">
          ...
          <li>
            6. 待上方命令执行完毕后，Tapdata Agent
            会处于启动状态，你可以通过以下命令对 Tapdata Agent 进行管理
          </li>
        </ul>
      </template>
      <template v-if="downLoadType === 'Linux'">
        <ul class="pt-5 ul-style">
          <li>1. 部署前请确认你的本地环境中已安装 java</li>
          <li>
            2. 请复制下方命令并在本地部署环境执行，其包含 Tapdata Agent
            的下载、自动部署及启动
          </li>
      ...
      <template v-if="downLoadType === 'Docker'">
        <ul class="pt-5 ul-style">
          ...
          <li>
            2.
            请复制下方命令并在本地部署环境执行，其包含镜像的下载及运行，Tapdata
            Agent 的下载、自动部署及启动
          </li>
          <li>3. 下载、部署 Tapdata Agent 无需 root 权限</li>
          <li>4. 我们非常建议在一个独立、干净的文件夹内部署 Tapdata Agent</li>
