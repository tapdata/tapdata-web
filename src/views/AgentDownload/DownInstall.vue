    ...
    <main class="page-main block">
      ...
      <template v-if="type !== 'Docker'">
        <p class="title-text pt-3">
          tapdata agent 的运行依赖本地 java 环境，因此，在部署 agent
          前你需要检查本地是否已经部署 java 环境，java
          环境的检测与安装办法请根参考下方提示：
        </p>
      ...
      <div v-else>
        <p class="title-text pt-3">
          在下一步的安装步骤中，我们提供了包含 java 环境的 docker
          镜像，如果你希望使用自己 的镜像但其缺少 java 环境，请联系你的 docker
          镜像管理员，或前往下方的 docker 官网寻 找对应镜像
        </p>
      ...
      <template v-if="type === 'Linux'">
        ...
        <div class="text-style pt-8">
          可参考下方的命令为本地安装 java 环境（本提示中的 openjdk
          版本仅用于示例）：
        </div>
