<template>
  <section
    ...
    v-loading.fullscreen="!productMap || ($route.params.id && !agentInfo)"
  >
        ...
        >
          <div class="panel" v-if="$route.params.id && agentInfo">
            ...
            <ul class="info-list">
              ...
              <li class="info-item">
                ...
                <span class="value"
                  >{{ agentInfo.regionFmt }} | {{ agentInfo.zoneFmt }}</span
                >
              </li>
              <li class="info-item">
                <span class="label">实例规格</span>
                <span class="value">{{
                  specMap[agentInfo.spec.specType]
                }}</span>
              </li>
              <li class="info-item">
                <span class="label">同步拓扑</span>
                <span class="value">{{
                  topologyMap[agentInfo.spec.direction].split('（')[0]
                }}</span>
              </li>
              <li class="info-item">
                ...
                <span class="value">{{
                  chargeMap[
                    agentInfo.orderInfo.chargingMode +
                      ',' +
                      agentInfo.orderInfo.periodType
                  ]
                }}</span>
              </li>
              <li class="info-item">
                ...
                <span class="value">{{
                  $moment(agentInfo.createAt).format('YYYY-MM-DD HH:mm:ss')
                }}</span>
              </li>
              <li
                class="info-item"
                v-if="isInternet && agentInfo.orderInfo.periodType === 'month'"
              >
          ...
          <div class="panel" v-if="$route.name !== 'Renew'">
            ...
            <ElFormItem label="同步拓扑">
              <ElRadioGroup
                class="radio-button"
                :value="form.spec.direction"
                @input="handlerSyncChange"
              >
                <ElRadioButton
          ...
          <div class="panel" v-if="$route.name === 'Renew'">
            ...
            <ElFormItem label="续订后到期时间">
              <span style="font-size: 12px; color: #f04134">{{
      ...
      <template v-if="pageType === 'confirm'">
        ...
        <div class="panel">
          <ElTable ref="table" :data="[{}]">
            <ElTableColumn>
              ...
              <template>
                <div style="padding-left: 20px">
                  数据库复制DRS
                  <span v-if="$route.name === 'Modify'">规格变更</span
                  ><span v-if="$route.name === 'Renew'">续订</span>
                </div>
              </template>
            </ElTableColumn>
            <ElTableColumn label="配置信息" min-width="150px">
              <template>
                <div class="props-item">
                  <span class="label">地域及可用区</span>
                  <span class="value"
                    >{{ regionMap[form.region] }} |
                    {{ zoneMap[form.zone] }}</span
                  >
                </div>
                <div class="props-item">
                  <span class="label">同步拓扑</span>
            ...
            <ElTableColumn label="计费模式">
              <template>{{
                chargeMap[form.chargingMode + ',' + form.periodType]
              }}</template>
            </ElTableColumn>
            <ElTableColumn
              label="时长"
              v-if="
                user.customerType !== '政企集团客户' &&
        ...
        <div class="panel confirm-footer">
          ...
          <div class="agreement">
            ...
            <ElLink size="mini" type="primary" @click="toDoc()"
              >《移动云产品销售协议》</ElLink
            >
          </div>
...
const getProductKey = item => {
  return ['chargingMode', 'periodType', 'specType', 'topology']
    .map(key => item[key])
    .join('_')
}
export default {
    ...
    return {
      ...
      form: {
        ...
        name:
          ...
          Math.floor(Math.random() * 16 * 1000),
        serviceId: '',
        groupId: '',
        measureId: '',
        periodType: 'month',
  ...
  methods: {
    getInstance(id, regionMap, zoneMap) {
      this.$axios.get('api/tcm/agent/' + id).then(data => {
        ...
        this.agentInfo.endTimeStr = data.endTime
          ? this.$moment(data.endTime).format('YYYY-MM-DD')
          : ''
      })
    },
    getRegion(cb) {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          if (
            ...
            JSON.stringify(this.form.spec) ===
              JSON.stringify(this.agentInfo.spec)
          ) {
    ...
    submit() {
      if (this.agree) {
        ...
        this.form.measureId = product.measureId
        let successTips =
          '您的资源已经开始创建，预计需要几分钟时间，您可以到实例列表查看，并进行下一步操作。'
        this[method[action]]()
          .then(data => {
            if (
              !data.products ||
              !data.products[0] ||
              !data.products[0].orderExtId
            ) {
    ...
    toChargeDetails() {
      window.open(
        process.env.VUE_APP_BASE_URL + '/op-help-center/doc/article/42279',
        '_blank'
      )
    }
  }
