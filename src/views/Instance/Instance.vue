  ...
  >
    <div class="main">
      <div class="instance-operation">
          ...
          <ul>
            ...
            <li style="margin-left: 10px;">
              ...
              >
                <i
                  slot="prefix"
                  class="iconfont td-icon-sousuo el-input__icon"
                ></i>
              </ElInput>
            </li>
            <li style="margin-left: 10px;">
        ...
        <div
          v-if="VUE_APP_HIDE_INSTANCE_BTN !== 'true'"
          class="instance-operation-right"
        >
      ...
      >
        <ElTableColumn min-width="200px" label="实例ID/名称">
          <template slot-scope="scope">
            <ElLink
              :class="['agent-link', $PLATFORM]"
              type="primary"
              @click="clickAgent(scope.row)"
              >{{ scope.row.id }}</ElLink
            >
        ...
        <ElTableColumn label="任务数" width="120">
          <template slot-scope="scope">
            <ElTooltip
              effect="dark"
              placement="top"
              :disabled="!scope.row.metric || !scope.row.metric.runningTaskNum"
            >
              <div slot="content">
        ...
        <div class="instance-table__empty" slot="empty">
          ...
          >
          <span v-else>
            没有查到符合条件的结果，<ElLink type="primary" @click="reset"
              >返回列表</ElLink
            >
          </span>
        </div>
...
export default {
  ...
  methods: {
          ...
          if (orderId) {
            window.open(
              process.env.VUE_APP_BASE_URL +
                `/usercenter/mop/order/orderdetail/${orderId}`,
              '_blank'
            )
          } else {
    ...
    toRenew(item, index) {
      ...
      window.open(
        process.env.VUE_APP_BASE_URL +
          process.env.VUE_APP_PURCHASE_PATH +
          '#/renew/' +
          item.id,
        '_blank'
      )
    ...
    toCancel(item, index) {
      ...
      if (item.metric && item.metric.runningTaskNum) {
        return this.$alert(
          '该实例已创建了同步任务或数据源，请先删除同步任务和数据源再进行退订操作',
          {
            type: 'warning',
      ...
      this.$confirm(
        '实例 ' + item.name + ' 退订后不能恢复，且账号不再对该实例计费',
        '是否退订该实例',
        {
          type: 'warning'
        }
