<template>
  <ElDialog
    ...
    custom-class="novice-guide-dialog"
  >
    <div
      v-if="myItem"
      :class="['guide-container', 'flex', 'flex-column', 'step' + step]"
      :style="myStyle"
    >
      <!-- 标题 -->
      <div
        v-if="myItem.title"
        :class="['dialog-header', 'py-5', myItem.title.class]"
        :style="myItem.title.style"
      >
        <template v-if="typeof myItem.title.text === 'object'">
          <div
            v-for="(item, index) in myItem.title.text"
            :key="index"
            :class="[myItem.title.itemClass]"
          >
            {{ item }}
          </div>
        </template>
        <template v-else :class="[myItem.title.itemClass]">{{
          myItem.title.text
        }}</template>
      </div>
      <!-- 描述 -->
      ...
      >
        <template v-if="typeof myItem.desc.text === 'object'">
          <div
            v-for="(item, index) in myItem.desc.text"
            :key="index"
            :class="[myItem.desc.itemClass]"
          >
            {{ item }}
          </div>
        </template>
        <template v-else :class="[myItem.desc.itemClass]">{{
          myItem.desc.text
        }}</template>
      </div>
      <!-- 内容 -->
      <div :class="['content-container', 'flex-grow-1', myItem.contentClass]">
        <!-- setion -->
        <div
          :class="['section-container', 'inline-flex', section.class]"
          v-if="section"
        >
          <div
            v-for="(seItem, seIndex) in section"
            :key="seIndex"
            :class="[seItem.class]"
            :style="seItem.style"
          >
            ...
            <div class="text-section" v-else>
              ...
              <template v-else :class="[seItem.itemClass]">{{
                seItem.text
              }}</template>
            </div>
          </div>
        ...
        <div
          ...
          :class="[imgList.class]"
          :style="imgList.style"
          v-if="imgList.items.length"
        >
          <div
            :class="[
              'img-box',
              { active: selectImg === item.key },
              imgList.itemClass
            ]"
          ...
          >
            ...
            <div
              :class="['img-box-name', imgList.imageTitleClass]"
              v-if="item.name"
              :style="item.imageTitleStyle"
            >
              {{ item.name }}
            </div>
            <el-button
      ...
      <div
        v-if="footerContent"
        :class="['footer-content-container', footerContent.class]"
      >
        <div v-for="(item, index) in footerContent.text" :key="index">
          {{ item }}
          <span
            v-if="
              footerContent.extra && index === footerContent.text.length - 1
            "
...
export default {
  ...
  computed: {
    ...
    myStyle() {
      ...
      if (bgImg) {
        style.background =
          'url(' + this.getImgByType(bgImg.key) + ') no-repeat 100% / cover'
      }
      return style
    }
  },
  ...
  methods: {
    ...
    goCreateTask() {
      this.show = false
      this.$router.push(
        '/dataflow/create?step=' + this.getListIndexByKey('dataSource_1')
      )
    },
    goDashboard() {
      this.show = false
