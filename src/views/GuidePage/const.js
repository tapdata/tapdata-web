  ...
  },
  {
    ...
    footerContent: {
      ...
      extra: {
        ...
        to:
          'https://docs.cloud.tapdata.net/chan-pin-jian-jie/zhi-chi-de-shu-ju-yuan-lei-xing'
      }
    }
  },
  // 继续添加数据源、创建数据同步任务
...
export async function getConst() {
  return await window.axios
    ...
    .then(data => {
      ...
      allowDataType =
        data
          .find(item => item.key === 'ALLOW_CONNECTION_TYPE')
          ?.value?.split(',') || []
      // 不支持的数据类型
      comingAllowDataType =
        data
          .find(item => item.key === 'COMING_ONLINE_CONNECTION_TYPE')
          ?.value?.split(',') || []
      STEPS.forEach(el => {
        if (el.key === 'dataSource_select') {
          el.imgList.items = getDataTypeItems(
            allowDataType,
            comingAllowDataType
          )
        }
