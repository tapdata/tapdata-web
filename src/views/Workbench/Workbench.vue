<template>
	<div v-if="$route.name === 'Workbench'" class="workbench-container flex flex-grow-1">
		<div class="main flex-grow-1">
			<div class="main-header py-6 px-7">
				...
				<div class="main-subtitle fs-7">
					<span class="mr-4">安全、简单、永久免费的实时数据流融合服务 从这里开始您的数据旅途</span>
					<img src="../../assets/image/workbench/arrow_right.png" alt="" />
					<span class="ml-2 text-decoration-underline pointer" @click="showGuide">开启引导</span>
				</div>
			</div>
			<div class="main-content">
				<div class="create-list py-6 px-10">
					<ul class="create-list__list flex justify-between">
						<li v-for="(item, index) in createList" :key="index" class="create-list__item">
							...
							<div class="create-list__title mb-4 fs-8 text-wrap">{{ item.title }}</div>
							<el-button
				...
				<ul class="introduction-list justify-between flex flex-wrap px-6 py-4">
					<li v-for="(item, index) in introductionList" :key="index" class="introduction-list__item p-4">
						<div class="introduction-list__box flex p-4 pointer" @click="clickIntro(item)">
							<div class="introduction-list__icon">
								<img :src="getIconSrc(item.icon)" alt="" />
							</div>
							<div class="introduction-list__content ml-4">
								<div class="introduction-list__name mb-3 fs-7">{{ item.name }}</div>
								<div class="introduction-list__title" v-for="(t, i) in item.title" :key="i">{{ t }}</div>
							</div>
						</div>
					</li>
				</ul>
			...
			<div class="notice mb-6">
				...
				<div class="notice-list px-5">
					<ul class="notice-list__list pt-3">
						...
						>
							<div v-if="item.type" class="notice-list__type mr-4 p-1">{{ item.type }}</div>
							<div class="notice-list__name flex-grow-1 ellipsis pointer" @click="toNotice">{{ item.name }}</div>
				...
				<div class="guide-list px-5">
					<ul class="guide-list__list pt-3">
						...
						>
							...
							<div class="guide-list__name ml-4 flex align-center">{{ item.name }}</div>
						</li>
					</ul>
				</div>
			</div>
...
.workbench-container {
	...
	.main {
		...
		.main-header {
			color: #fff;
			background: url(../../assets/image/workbench/workbench_bg.png) no-repeat 100% / cover;
