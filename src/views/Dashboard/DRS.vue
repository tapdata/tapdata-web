          ...
          >
            ...
            <ElLink
              :class="[
                ...
                { 'is-danger': ['Error', 'Freeze'].includes(key) }
              ]"
              type="primary"
              @click="
                toInstance({ region: item.poolId, status: key, zone: zone.id })
              "
              >{{ value }}</ElLink
            >
          </span>
...
export default {
  ...
  methods: {
    ...
    toPurchase() {
      window.open(
        process.env.VUE_APP_BASE_URL + process.env.VUE_APP_PURCHASE_PATH,
        '_blank'
      )
    },
    toInstance(query) {
