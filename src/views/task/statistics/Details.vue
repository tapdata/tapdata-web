<template>
  <el-container v-loading="loading" class="task-details-container flex-column" v-if="$route.name === 'DataflowDetails'">
        ...
        <ElTabPane label="子任务" name="subTask" class="h-100">
          ...
          <ElTable :data="list" class="mt-2">
            ...
            <ElTableColumn label="操作">
              <template v-slot="scope">
                <ElButton type="text" @click="handlePause([scope.row.id], scope.row)"> 暂停 </ElButton>
                <ElButton
                ...
                <ElButton type="text" @click="handleRunStatisitics(scope.row.id)">运行统计</ElButton>
              </template>
            </ElTableColumn>
          </ElTable>
        </ElTabPane>
        <ElTabPane lazy class="h-100 overflow-hidden" label="连接" name="connection">
          <Connection :task="task" @change="loadTask"></Connection>
        </ElTabPane>
        <ElTabPane lazy class="h-100 overflow-hidden" label="历史运行记录" name="history">
...
export default {
  name: 'TaskDetails',
  components: { StatusTag, VIcon, InlineInput, Connection },
  data() {
    return {
      loading: true,
      activeTab: 'subTask',
  ...
  methods: {
    ...
    handleRunStatisitics(id) {
      ...
      // this.$router.push({ name: 'DataflowStatistics' })
    }
  }
}
</script>
